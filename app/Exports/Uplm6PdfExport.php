...
class Uplm6PdfExport implements FromCollection, WithHeadings, WithMapping
{
    protected $jenis;
    protected $subjenis;
    protected $tahun;
        ...
        if ($this->tahun) {
            $query->whereHas('jawaban', function ($q) {
                $q->whereHas('pertanyaan', function ($qPertanyaan) {
                    $qPertanyaan->where('tahun', $this->tahun)->where('kategori', 'UPLM 6');
                });
            });
        }
    ...
    public function headings(): array
    {
        ...
        // Ambil pertanyaan khusus untuk UPLM 6 dan filter berdasarkan tahun
        $pertanyaan = Pertanyaan::where('kategori', 'UPLM 6')
    ...
    public function map($item): array
    {
        ...
        // Ambil pertanyaan UPLM 6 dan filter berdasarkan tahun
        $pertanyaan = Pertanyaan::where('kategori', 'UPLM 6')
            ->when($this->tahun, function ($query) {
                $query->where('tahun', $this->tahun);
            })
    ...
    public function downloadPdf()
    {
        ...
        // Ambil pertanyaan UPLM 4 dan filter berdasarkan tahun
        $pertanyaan = Pertanyaan::where('kategori', 'UPLM 6')
            ->when($this->tahun, function ($query) {
                $query->where('tahun', $this->tahun);
            })
        ...
        $pdf = Pdf::loadView('admin.uplm6_pdf', compact('data', 'headings', 'pertanyaan'))
            ->setPaper('a4', 'landscape');

        return $pdf->download('uplm6-report.pdf');
    }
}