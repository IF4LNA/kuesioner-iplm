...
class Uplm7PdfExport implements FromCollection, WithHeadings, WithMapping
{
        ...
        if ($this->tahun) {
            $query->whereHas('jawaban', function ($q) {
                $q->whereHas('pertanyaan', function ($qPertanyaan) {
                    $qPertanyaan->where('tahun', $this->tahun)->where('kategori', 'UPLM 7');
                });
            });
        }
    ...
    public function headings(): array
    {
        ...
        // Ambil pertanyaan khusus untuk UPLM 7 dan filter berdasarkan tahun
    ...
    public function downloadPdf()
    {
        ...
        $pdf = Pdf::loadView('admin.uplm7_pdf', compact('data', 'headings', 'pertanyaan'))
