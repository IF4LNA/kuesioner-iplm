...
class Uplm3PdfExport implements FromCollection, WithHeadings, WithMapping
{
    protected $jenis;
    protected $subjenis;
    protected $tahun;
        ...
        if ($this->tahun) {
            $query->whereHas('jawaban', function ($q) {
                $q->whereHas('pertanyaan', function ($qPertanyaan) {
                    $qPertanyaan->where('tahun', $this->tahun)->where('kategori', 'UPLM 3');
                });
            });
        }
    ...
    public function headings(): array
    {
        ...
        // Ambil pertanyaan khusus untuk UPLM 3 dan filter berdasarkan tahun
    ...
    public function map($item): array
    {
        ...
        // Ambil pertanyaan UPLM 3 dan filter berdasarkan tahun
        $pertanyaan = Pertanyaan::where('kategori', 'UPLM 3')
            ->when($this->tahun, function ($query) {
                $query->where('tahun', $this->tahun);
            })
