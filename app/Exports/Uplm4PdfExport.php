...
class Uplm4PdfExport implements FromCollection, WithHeadings, WithMapping
{
    protected $jenis;
    protected $subjenis;
    protected $tahun;
        ...
        if ($this->tahun) {
            $query->whereHas('jawaban', function ($q) {
                $q->whereHas('pertanyaan', function ($qPertanyaan) {
                    $qPertanyaan->where('tahun', $this->tahun)->where('kategori', 'UPLM 4');
                });
            });
        }
    ...
    public function headings(): array
    {
        ...
        // Ambil pertanyaan khusus untuk UPLM 4 dan filter berdasarkan tahun
    ...
    public function downloadPdf()
    {
        ...
        $pdf = Pdf::loadView('admin.uplm4_pdf', compact('data', 'headings', 'pertanyaan'))
            ->setPaper('a4', 'landscape');

        return $pdf->download('uplm4-report.pdf');
    }
}